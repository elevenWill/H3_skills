...
3. **30 秒及以上完整 MV 版（多镜头拼接合成）**：许多模型的单次生成上限可能在约 15 秒，系统将自动采用 **“多分镜（Multi-Shot）拼接工作流”**。先生成或锁定一首完整连续歌曲 / BGM，将 30 秒拆解为 4~8 个 2~5 秒的动态分镜头，通过卡拍（Beat-Sync）与首尾帧/场景连续性控制，拼接合成为无缝 MV。
