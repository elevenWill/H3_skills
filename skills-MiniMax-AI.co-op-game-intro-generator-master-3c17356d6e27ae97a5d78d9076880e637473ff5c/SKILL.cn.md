...
以下两份模板是运行时必需输入，不是可选说明：

- 在 STEP 3 生成确认图提示词并生成首张确认图（STEP 4）时，必须使用 `references/h3-confirmation-image-template.md`，按顺序填写模板字段，不得跳过框架、色板、UI、角色、字体、布局或负向约束项。
- 在 STEP 6 回填最终 Minimax H3 视频 prompt 时，必须使用 `references/h3-video-prompt-template.md`，并以确认图、玩家/游戏信息、最终 UI 文案、事件节奏、动作方向和负向约束为填充来源。

如果任一模板文件不存在，请停止并报告技能包不完整，不要改写为其他 prompt 结构。
...
## STEP 3：生成 GPT 首图提示词
先加载 `references/h3-confirmation-image-template.md` 并严格遵循其骨架。GPT 首图提示词必须采用“框架固定 + 风格动态填充 + 颜色联动”的写法：
...
首图确认后，先加载 `references/h3-video-prompt-template.md` 再回填确认结果至完整视频 prompt，包括风格、角色参考、玩家名字、游戏名字、UI 文案和负面约束，然后直接用 Minimax H3 生成最终视频。
